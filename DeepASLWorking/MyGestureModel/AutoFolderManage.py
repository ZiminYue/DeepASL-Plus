...
def split_dataset(src_dir, dest_dir, train_size=50, val_size=20):
    ...
    for class_name in classes:
        ...
        if not os.path.isdir(class_path):  # Only manage folders 只处理文件夹
            continue

        # Create taining set and validation set folders 创建训练集和验证集文件夹 
        ...
        # Get all images from this catagory 获取当前类别的所有图片
        images = os.listdir(class_path)
        random.shuffle(images)  # 打乱顺序

        # Divide them into training set and validation set at 50/20 按照指定的数量分配训练集和验证集
        train_images = images[:train_size]
        val_images = images[train_size:train_size+val_size]

        # Copy training set images 复制训练集图片
        for img in train_images:
            shutil.copy(os.path.join(class_path, img), os.path.join(train_class_dir, img))

        # Copy validation set images 复制验证集图片
        for img in val_images:
            shutil.copy(os.path.join(class_path, img), os.path.join(val_class_dir, img))

    print("Dataset folder managed！")

# Dataset Source Path 数据集源路径
source_directory = './My Model/aslDataSet'

# Dataset target path 数据集目标路径
destination_directory = './My Model/aslDataSet4Training'

# Call the split function (50 training sets and 20 validation sets per category) 调用拆分函数（每个类别 50 张训练集，20 张验证集）
